...
def bfs_one_way_router(coordinates_dict, connection_list, grid_dimen, layers=1):
    """ 
    ...
    for i, (source_id, dest_id) in enumerate(connections):
        ...
        path = bfs_one_way_search(source_pos, dest_pos, blocked_nodes=temp_obstacles, layers=layers, grid=grid_dimen, nodes=nodes)
...
def bfs_one_way_search(start, goal, blocked_nodes, layers, grid, nodes):
    """
    start: (x, y, z)
    goal: (x, y, z)
    """
...
if __name__ == "__main__":
    ...
    nets = bfs_one_way_router(points, connections, (-10, -10, 10, 10), layers=4)
    print("Nets: ", nets)
